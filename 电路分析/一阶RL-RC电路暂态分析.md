...
电路：10V 电源，串联 100Ω 电阻后到节点 N；N 引出两条并联支路到公共下端——一条是 100Ω 电阻串联开关 S，另一条是 0.2H 电感（ $i_L(t)$ ）。S 在 $t=0$ 时**闭合**（闭合前 S 断开）。

1. **0₋ 时刻**：S 断开，"100Ω+S" 支路不通，电路简化为电源串联 100Ω 再接电感的单回路，电感相当于短路：
   $$i_L(0_-) = \frac{10}{100} = 0.1\ \text{A} = 100\ \text{mA}$$
2. **换路定则**：
   $$i_L(0_+) = i_L(0_-) = 0.1\ \text{A} = 100\ \text{mA}$$

> [!NOTE]
> S 闭合后 100Ω(右) 支路接入，电路会按新的时间常数朝新稳态变化，但 $i_L(0_+)$ 本身由换路**前**的电路决定，不受换路瞬间新电路的影响。
