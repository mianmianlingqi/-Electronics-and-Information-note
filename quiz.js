(function () {
  ...
  const STORAGE_KEY = 'electronics-quiz-v1';
  const INTERVALS = {
    bad: 10 * 60 * 1000,
    mid: 24 * 60 * 60 * 1000,
  ...
  function setCardRating(id, rating) {
    ...
    const prev = prog[id] || { streak: 0, interval: 0 };
    let nextReview;
  ...
  function readLink(card) {
    const anchor = card.readAnchor || 'circuit';
  ...
  function updateBadges() {
    ...
    $('badgeReview').parentElement.classList.toggle('primary', due > 0);
  }
  ...
  function bindEvents() {
    ...
    $('btnBackHome2').addEventListener('click', e => {
      e.preventDefault();
      renderHome();
    });
    ...
    $('btnRetryWrong').addEventListener('click', () => {
      const ids = new Set(
        state.session.rated.filter(r => r.rating !== 'good').map(r => r.card.id)
      );
      state.queue = state.session.rated
  ...
  function parseUrlParams() {
    ...
    if (chapter && state.data) {
      const ch = state.data.chapters.find(c => c.readAnchor === chapter || c.id === chapter);
      if (ch) {
        state.selectedChapters.clear();
